...
def main():
    ...
    # 1) VAD : Segmentation (Ton fichier s'appelle vad_segment.py)
    run("python TP3/vad_segment.py")
    vad = load_json("TP3/outputs/vad_segments_call_01.json")

    # 2) ASR : Transcription (Ton fichier s'appelle asr_whisper.py)
    run("python TP3/asr_whisper.py")
    asr = load_json("TP3/outputs/asr_call_01.json")

    # 3) Analytics : PII et Intentions (Ton fichier s'appelle callcenter_analytics.py)
